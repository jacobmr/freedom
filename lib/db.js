...
export async function upsertAccount(email, shareId) {
  ...
  if (!e || !shareId) return;
  // First-claim-wins: never overwrite an email already bound to a share. Email is
  // unverified at save time, so overwriting would let anyone hijack another user's
  // email -> adventure mapping. (Full email verification is a future enhancement.)
  await db().execute({
    sql: 'INSERT INTO accounts (email, share_id, updated_at) VALUES (?,?,?) ON CONFLICT(email) DO NOTHING',
